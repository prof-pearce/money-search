...
[
  ...
  no-propensities      ; tendencies to not speculate on each good
  yes-propensities     ; tendencies to speculate on each good
  trade-probabilities  ; speculating probabilities for each good
  regret               ; regret over my speculation choice
  partner              ; my current trade partner
  previous-good        ; good offered in last trade attempt
  yes-payoffs          ; payoffs if I always trade per good
  no-payoffs           ; payoffs if I never trade per good
...
to init-trader
  ...
  set no-propensities n-values num-goods [0]
  set yes-propensities n-values num-goods [0]
  set trade-probabilities n-values num-goods [.5]
  ;set regret 1
  set regret payoff-sensitivity
  set shape "Person"
  set partner nobody
  set yes-payoffs n-values num-goods [0]
  set no-payoffs n-values num-goods [0]
  set previous-good good-produced
  set participated? false
  set speculate? map[[p] -> random-float 1 < p] trade-probabilities
end
...
; begin a new series
to reset-trader
  set no-propensities (map weighted-avg (map not speculate?) no-propensities)
  set yes-propensities (map weighted-avg speculate? yes-propensities)
  let exp-yes-propensities map exp-propensity yes-propensities
  let exp-no-propensities map exp-propensity no-propensities
  set trade-probabilities (map [[y n] -> y / (y + n)] exp-yes-propensities exp-no-propensities)
...
to-report will-trade? [offered-good]
  ...
  ; maybe I'll take it
  report item offered-good speculate?
end
...
to-report weighted-avg [spec? old-avg]
  report ifelse-value spec? [(1 - weight) * old-avg + weight * series-payoff][old-avg]
end

to-report exp-propensity [propensity]
  report exp (payoff-sensitivity * propensity / regret)
end

to-report avg-utility [trader-type]
  let total 0
  ask traders with [good-produced = trader-type] [set total total + utility]
...
"" ""
PENS
"salt1" 1.0 0 -11085214 true "" "plot avg-trade-probability 0 0"
"sugar1" 1.0 0 -2674135 true "" "plot avg-trade-probability 0 1"
"spice1" 1.0 0 -14454117 true "" "plot avg-trade-probability 0 2"
...
"" ""
PENS
"salt" 1.0 0 -11085214 true "" "plot avg-trade-probability 1 0"
"sugar" 1.0 0 -2674135 true "" "plot avg-trade-probability 1 1"
"spice" 1.0 0 -13345367 true "" "plot avg-trade-probability 1 2"
...
"" ""
PENS
"salt" 1.0 0 -13840069 true "" "plot avg-trade-probability 2 0"
"sugar" 1.0 0 -2674135 true "" "plot avg-trade-probability 2 1"
"spice" 1.0 0 -13345367 true "" "plot avg-trade-probability 2 2"
